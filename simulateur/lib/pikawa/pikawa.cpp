...
SSD1306Wire display(ADRESSE_I2C_OLED,
                    I2C_SDA_OLED,
                    I2C_SCL_OLED); //!< l'objet OLED
String      entete =
  String(EN_TETE) + String(DELIMITEUR_CHAMP);    //!< entête d'une trame pikawa
...
bool   changementCommandeCafe =
  false; //!< indique un changement de commande du café
EtatTasse etatTasse =
  (EtatTasse)Inconnu; //!< l'état de la détecion d'une tasse (état simulé)
EtatBac etatBac =
  (EtatBac)Inconnu; //!< l'état du bac de récupération de capsules (état simulé)
EtatCommande etatCommande =
  (EtatCommande)Inconnu; //!< l'état de la préparation d'un café (la dure de
                         //!< préparation est simulée)
EtatNiveauEau etatNiveauEau =
  (EtatNiveauEau)Inconnu; //!< l'état du niveau d'eau (état simulé)
EtatCapsule etatCapsule =
...
int contenanceBac = 0; //!< pour définir la contenance en nombre de capsules du
                       //!< bac de récupération
int contenanceEau =
  CAPACITE_EAU; //!< pour simuler le niveau d'eau d'une machine à café
...
const int longueurLigne =
  42; //!< longueur de la ligne 6 (pour gérer un dépassement)
...
void traiterTrames()
{
    ...
    if(lireTrame(trame))
    {
        ...
        else if(typeTrame == String(TRAME_COMMANDE_PREPARATION))
        {
            ...
            if(traiterCommandeCafe(extraireChamp(trame, 3),
                                   extraireChamp(trame, 2)))
            {
...
String verifierTrame(String& trame)
{
    ...
    {
        if(compterParametres(trame) ==
           NB_PARAMETRES_TRAME_REQUETE_ETAT_CAFETIERE)
...
bool estMagasinVide()
{
    for(unsigned int numeroColonne = 0; numeroColonne < NB_COLONNES;
        ++numeroColonne)
    {
...
bool verifierEtatsMachine(int numeroColonne, String longueurCafe)
{
    ...
    if(etatNiveauEau == Vide ||
       ((contenanceEau - getNiveauNecessaire(longueurCafe)) < 0))
    {
#ifdef DEBUG
...
bool commanderCafe(int etat)
{
    ...
    unsigned long dureeCafe =
      (unsigned long)TEMPO_CMD_CAFE * (unsigned long)longueurCafeCommande;
    ...
    if(estEcheance(TEMPO_SIMULATION))
    {
        ...
        switch(etatSimulation)
        {
            case Tasse:
                ...
                {
                    taus =
                      random(0, 10); // 1 fois sur 10 pour simuler Tasse absente
                    if(taus == 0)
                    {
...
void setLigne3()
{
    if(etatCommande == EnCours)
    {
        if(longueurCafeCommande != (LongueurCafe)OFF &&
           numeroCapsuleCommande != -1)
        {
            char message[64] = "";
            if(longueurCafeCommande == Long)
                ...
                sprintf((char*)message,
                        "Cafe %d moyen : encours\r\n",
                        numeroCapsuleCommande);
            else
                sprintf((char*)message,
                        "Cafe %d court : encours\r\n",
                        numeroCapsuleCommande);
